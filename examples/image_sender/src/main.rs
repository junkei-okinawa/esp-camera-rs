...
impl MacAddress {
    ...
    fn config_rs_mac_address(&self) -> String {
        format!("[0x{:02x}, 0x{:02x}, 0x{:02x}, 0x{:02x}, 0x{:02x}, 0x{:02x}]",
...
fn main() -> anyhow::Result<()> {
    ...
    // 受信機の MAC アドレスをパース
    let receiver_mac = MacAddress::from_str(&app_config.receiver_mac).expect("Failed to parse receiver MAC address from cfg.toml");
    println!("Receiver MAC address: {}", receiver_mac);
    // 受信機の MAC アドレスを表示
    println!("Receiver MAC address: {:?}", receiver_mac.config_rs_mac_address());
    ...
    loop {
        ...
        if let Some(framebuffer) = camera.get_framebuffer() {
            ...
            SEND_FAILED.store(false, Ordering::SeqCst);
            unsafe {
                let result = esp_now_send(receiver_mac.0.as_ptr(), hash_payload.as_ptr(), hash_payload.len());
                if result != 0 {
            ...
            if !SEND_FAILED.load(Ordering::SeqCst) {
                ...
                for chunk in data.chunks(chunk_size) {
                    ...
                    unsafe {
                        let result = esp_now_send(receiver_mac.0.as_ptr(), chunk.as_ptr(), chunk.len());
                        if result != 0 {
                ...
                if SEND_FAILED.load(Ordering::SeqCst) {
                    ...
                    unsafe {
                        let result = esp_now_send(receiver_mac.0.as_ptr(), eof_marker.as_ptr(), eof_marker.len());
                        if result != 0 {
